...
Claude will run:
python3 $SKILL_DIR/scripts/generate_with_preset.py "Modern fintech hero image with abstract flowing gradients in blue and purple, suggesting financial growth and technology, clean minimalist style" "./src/assets/hero.jpg"
```
...
Claude will run:
python3 $SKILL_DIR/scripts/generate_with_preset.py --size 512 "Minimalist weather app icon, stylized sun partially behind a cloud, warm orange and blue gradient, rounded square suitable for iOS/Android" "./assets/icons/weather-icon.jpg"
```
...
Claude will run:
python3 $SKILL_DIR/scripts/generate_with_preset.py "E-commerce product placeholder image, neutral gray background, subtle product box outline, professional and clean" "./public/images/placeholder-product.jpg"
```
...
Claude will run:
python3 $SKILL_DIR/scripts/generate_with_preset.py --size 2K "Seamless tileable subtle geometric pattern, very light gray on white, minimalist, suitable for website background" "./assets/bg-pattern.jpg"
```
...
Claude will run:
python3 $SKILL_DIR/scripts/generate_with_preset.py "Clean technical diagram showing microservices architecture, boxes connected by arrows, services labeled API Gateway, Auth Service, User Service, Order Service, white background, professional style" "./docs/images/architecture.jpg"
```
...
Claude will run:
python3 $SKILL_DIR/scripts/generate_with_preset.py "Flowchart diagram showing user onboarding process, start with signup, branches for email verification, profile setup, tutorial completion, clean business style with blue accents" "./docs/onboarding-flow.jpg"
```
...
Claude will run:
python3 $SKILL_DIR/scripts/generate_with_preset.py "Modern product launch banner 1200x675, bold typography space for text overlay, dynamic abstract tech background, vibrant gradients purple to blue" "./marketing/twitter-banner.jpg"
```
...
Claude will run:
python3 $SKILL_DIR/scripts/generate_with_preset.py "Blog header image about artificial intelligence, abstract neural network visualization, glowing nodes and connections, dark background with blue accent lights, futuristic and professional" "./blog/images/ai-header.jpg"
```
...
Claude will run:
python3 $SKILL_DIR/scripts/generate_with_preset.py "Casual lifestyle portrait, Kodak Portra 400 color tones, natural window light with soft shadows, 3/4 body shot, authentic film grain, early-2000s digital camera aesthetic, relaxed pose, natural skin texture, warm highlights, slight background blur" "./branding/casual-portrait.jpg"
```
...
python3 $SKILL_DIR/scripts/generate_with_preset.py "A peaceful zen garden"

# Specify output location
python3 $SKILL_DIR/scripts/generate_with_preset.py "Mountain landscape" "./wallpapers/mountains.jpg"

# High resolution output
python3 $SKILL_DIR/scripts/generate_with_preset.py --size 2K "Detailed cityscape" "./high-res/city.jpg"

# Quick thumbnail/icon
python3 $SKILL_DIR/scripts/generate_with_preset.py --size 512 "Simple checkmark icon" "./icons/check.jpg"
```
...
$env:IMAGE_SIZE = "2K"
python3 $SKILL_DIR/scripts/generate_with_preset.py "Detailed cityscape" "./high-res/city.jpg"
```
...
for i in 1 2 3; do
    python3 $SKILL_DIR/scripts/generate_with_preset.py "Abstract art variation $i" "./art/abstract-$i.jpg"
done
```
...
# Generate multiple variations
1..3 | ForEach-Object {
    python3 $SKILL_DIR/scripts/generate_with_preset.py "Abstract art variation $_" "./art/abstract-$_.jpg"
}
```
...
Claude will:
1. Generate the image: python3 $SKILL_DIR/scripts/generate_with_preset.py "Minimal loading spinner, circular design, gradient from light to dark blue, clean vector style" "./src/assets/spinner.jpg"
...
Claude will:
1. Generate: python3 $SKILL_DIR/scripts/generate_with_preset.py "Friendly empty state illustration, person looking at empty box, soft pastel colors, friendly and approachable, modern SaaS style" "./src/assets/empty-state.jpg"
